        ...
        <script>
            if (typeof addOrRemoveCrudCheckedItem !== 'function') {
                function addOrRemoveCrudCheckedItem(element) {
                    ...
                    document.querySelectorAll('input.crud_bulk_actions_line_checkbox').forEach(checkbox => checkbox.onclick =
                        e => {
                            e.stopPropagation();
                            ...
                            if (checked) {
                                ...
                                if (crud.lastCheckedItem && e.shiftKey) {
                                    ...
                                    let first = document.querySelector(
                                        `input.crud_bulk_actions_line_checkbox[data-primary-key-value="${crud.lastCheckedItem}"]`
                                    ).closest('tr');
                                    let last = document.querySelector(
                                        `input.crud_bulk_actions_line_checkbox[data-primary-key-value="${primaryKeyValue}"]`
                                    ).closest('tr');
            ...
            if (typeof addBulkActionMainCheckboxesFunctionality !== 'function') {
                function addBulkActionMainCheckboxesFunctionality() {
                    ...
                    mainCheckboxes.forEach(checkbox => {
                        ...
                        checkbox.checked = document.querySelectorAll(
                            'input.crud_bulk_actions_line_checkbox:not(:checked)').length === 0;

                        // when the crud_bulk_actions_general_checkbox is selected, toggle all visible checkboxes
                        checkbox.onclick = event => {
                            rowCheckboxes.filter(elem => checkbox.checked !== elem.checked).forEach(elem => elem
                                .click());
                    ...
                    document.querySelectorAll('table td.dtr-control a').forEach(link => link.onclick = e => e
                        .stopPropagation());
                }
            }
            ...
            if (typeof enableOrDisableBulkButtons !== 'function') {
                function enableOrDisableBulkButtons() {
                    document.querySelectorAll('.bulk-button').forEach(btn => btn.classList.toggle('disabled', !crud.checkedItems
                        ?.length));
                }
            }
